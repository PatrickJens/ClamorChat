        ...
        else
        {
            dbManager.selectUserFromDB(landingPageUser.username).then(function(rows)
            {
                if(validateRows(rows) && authenticateLandingPageUser(rows[0]))
                {
                    ...
                    session_cookie = sessionManager.sessionToCookie(session); //sid=123;
                    console.log("LoginController session_cookie:", session_cookie);
                                        
                    //Broadcast new set of session users
                    ...
                    {   
                        ...
                        sessionManager.io.emit('updateNumUsers', 7);
                    });
                }
                else
                {
                    console.log("[LoginController] Login Invalid");
                    response.write("[LoginController.login]: Login Failure");
                    response.end();
                }
            });
        }   
